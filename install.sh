...
tls_config() {
	...
	echo
	echo -e "$yellow 请将 $magenta$wwwdomain$none $yellow解析到: $cyan$ip$none"
	echo
	echo "----------------------------------------------------------------"
	echo
...
domain_check() {
	...
	test_wwwdomain=$(curl -sH 'accept: application/dns-json' "https://cloudflare-dns.com/dns-query?name=$wwwdomain&type=A" | grep -oE "([0-9]{1,3}\.){3}[0-9]{1,3}" | head -1)
	...
	if [[ $test_wwwdomain != $ip ]]; then
		echo
		echo -e "$red 检测域名解析错误....$none"
		echo
		echo -e " 你的域名: $yellow$wwwdomain$none 未解析到: $cyan$ip$none"
		echo
		echo -e " 你的域名当前解析到: $cyan$test_wwwdomain$none"
		echo
		echo "备注...如果你的域名是使用 Cloudflare 解析的话..在 Status 那里点一下那图标..让它变灰"
		echo
		exit 1
